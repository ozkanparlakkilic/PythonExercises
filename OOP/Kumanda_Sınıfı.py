...
class Kumanda():
    ...
    def kanal_degistir(self):
        ...
        while True:
            karakter = input("Geri gitmek için '<' İleri gitmek İçin '>'")
            for i in self.kanal_listesi:
                if self.kanal == i:
                    kanalNumarasi = self.kanal_listesi.index(i)

            if karakter == "<":
                if kanalNumarasi < 0:
                    kanalNumarasi = len(self.kanal_listesi) - 1
                    self.kanal = self.kanal_listesi[kanalNumarasi]
                ...
                if kanalNumarasi >= len(self.kanal_listesi) - 1:
                    kanalNumarasi = 0
                    self.kanal = self.kanal_listesi[kanalNumarasi]
                    print("Şu anki Kanal {}".format(self.kanal))
                else:
